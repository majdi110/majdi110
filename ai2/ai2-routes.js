...
function safeJoin(root, user){
  const p = path.normalize('/' + String(user || '').replace(/^\/+/, ''));
...
module.exports = function(app){
  ...
  app.post('/ai2', (req, res) => {
    ...
    try { fs.accessSync(QUEUE_DIR, fs.constants.W_OK); }
    catch { return res.status(500).json({ error: 'Queue not writable' }); }
    ...
    catch(e){ try { fs.unlinkSync(tmp); } catch {} return res.status(500).json({ error: 'Failed to enqueue job' }); }

    res.status(202).json({ queued: true, job: name, hint: 'worker.sh will apply, commit and push' });
  });

  // GET /ai2/fs/list?path=&depth=
  app.get('/ai2/fs/list', (req, res) => {
    if (!authOk(req, res)) return;
    const rel = String(req.query.path || '');
    let depth = parseInt(String(req.query.depth || '1'), 10);
